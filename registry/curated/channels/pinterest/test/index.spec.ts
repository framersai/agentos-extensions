/**
 * Unit tests for the Pinterest channel extension factory.
 */
...
describe('createExtensionPack', () => {
  beforeEach(() => {
    // Clean env vars to avoid leakage between tests
  ...
  it('should create a pack with the correct name and version', () => {
    ...
    expect(pack.name).toBe('@framers/agentos-ext-channel-pinterest');
  ...
  it('should set the channel descriptor ID to pinterestChannel', () => {
    const pack = createExtensionPack({
      options: { accessToken: 'test-token' },
    } as any);

    const channel = pack.descriptors.find((d) => d.kind === 'messaging-channel');
    expect(channel?.id).toBe('pinterestChannel');
  });
  ...
  it('should resolve token from secrets map', () => {
    const pack = createExtensionPack({
      options: { secrets: { 'pinterest.accessToken': 'secret-token' } },
    } as any);
    expect(pack.descriptors.length).toBe(7);
  });

  it('should resolve token from PINTEREST_ACCESS_TOKEN env var', () => {
    process.env.PINTEREST_ACCESS_TOKEN = 'env-token';
    const pack = createExtensionPack({
      options: {},
    } as any);
    expect(pack.descriptors.length).toBe(7);
  ...
  it('should resolve token from PINTEREST_TOKEN env var as fallback', () => {
    process.env.PINTEREST_TOKEN = 'env-fallback-token';
    const pack = createExtensionPack({
      options: {},
    } as any);
    expect(pack.descriptors.length).toBe(7);
  ...
  it('should use custom priority when provided', () => {
    const pack = createExtensionPack({
      options: { accessToken: 'test-token', priority: 75 },
    } as any);
  ...
  it('should call logger.info on activate if logger provided', async () => {
    ...
    await pack.onActivate!();
    expect(mockLogger.info).toHaveBeenCalledWith('[PinterestChannel] Extension activated');
  });
  ...
  it('should call logger.info on deactivate if logger provided', async () => {
    ...
    await pack.onActivate!();
    await pack.onDeactivate!();
    expect(mockLogger.info).toHaveBeenCalledWith('[PinterestChannel] Extension deactivated');
  });
});
